...
class ReaderWidget(QWidget):
    ...
    page_changed_signal = pyqtSignal(str, int, str)
    ...
    def load_document(self, file_path: str, current_page: int):
        try:
    ...
    def handle_selection(self, rect: QRect):
        ...
        offset_x = max(0, (self.pdf_viewer.width() - pixmap.width()) // 2)
        offset_y = max(0, (self.pdf_viewer.height() - pixmap.height()) // 2)
        ...
        pdf_rect = fitz.Rect(px0 / self.zoom_level, py0 / self.zoom_level, px1 / self.zoom_level, py1 / self.zoom_level)
        ...
        annots_to_delete = [annot for annot in page.annots() if annot.type[0] == fitz.PDF_ANNOT_HIGHLIGHT and annot.rect.intersects(pdf_rect)]
        
        if text or annots_to_delete:
            menu = QMenu(self)
            action_copy = menu.addAction("Copiar texto") if text else None
            action_highlight = menu.addAction("Destacar (Marca-texto)") if text else None
            action_remove = menu.addAction("Remover marcação") if annots_to_delete else None
            ...
            if action:
                if action == action_copy:
                    ...
                    page.add_highlight_annot(pdf_rect).update()
                    try: self.doc.saveIncr()
    ...
    def handle_mouse_zoom(self, zoom_in: bool):
